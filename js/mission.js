...
xhr.open('GET', 'weaponData.json', true);        // Prepare the request
...
xhr.onload = function() {                       // When readystate changes
  ...
  if(xhr.status === 200) {                      // If server status was ok
    ...
    //Create a new row with the data from the JSON file
        for (var i = 0; i < responseObject.weapons.length; i++) { // Loop through object
        Bag[i] = new weapon(responseObject.weapons[i].weaponId, responseObject.weapons[i].weaponName, responseObject.weapons[i].weaponBullet, responseObject.weapons[i].weaponDamage, responseObject.weapons[i].weaponPicture);
        showWeapon(Bag[i]);
        }
        console.log("load data from mydata.json file.");
    } else {
        console.log("Opps, can't load data from the server, please try again.");
    }
};
...
function draw() {
    ...
    if(gameOver == false) {
        ...
        if(enemyCount == 1) {
            ...
            pickEnemy = Math.floor((Math.random() * 8) + 1);
        }
